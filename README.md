...
Before building the Random Forest Classifier model, the `rated_recipes` dataset is split into training and testing sets using a 75/25 split. Additionally, to maintain consistency and keep the same train and test sets for the final model, `X` and `y` are stored with all the features of the dataset.
